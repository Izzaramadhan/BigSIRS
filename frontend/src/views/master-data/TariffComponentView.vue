...
const handleSave = async (data) => {
  ...
  try {
    if (editingItem.value) {
      await updateTariffComponent(editingItem.value.id, data);
    } else {
      await createTariffComponent(data);
    }
    closeModal();
    loadData();
  } catch (err) {
...
const handleToggleStatus = async (item) => {
  ...
  if (confirm(`Apakah Anda yakin ingin ${actionText} komponen tindakan "${item.name}"?`)) {
    try {
      await updateTariffComponentStatus(item.id, newStatus);
    } catch {
...
const handleDelete = async (item) => {
  if (confirm(`Apakah Anda yakin ingin menghapus komponen tindakan "${item.name}"?`)) {
    try {
      await deleteTariffComponent(item.id);
      loadData();
    } catch {
  ...
  <div class="page-container">
    <div class="page-header">
      <div class="header-content">
        <h1 class="page-title">Komponen Tindakan</h1>
        <p class="page-subtitle">Kelola master data komponen pembentuk tarif</p>
      </div>
    ...
    <div class="filter-bar">
      <div class="search-box">
        ...
        <input 
          ...
          v-model="searchQuery" 
          class="form-control" 
          placeholder="Cari nama atau deskripsi komponen..." 
        />
      </div>
