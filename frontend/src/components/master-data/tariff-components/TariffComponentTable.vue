      ...
      <thead>
        <tr>
          ...
          <th @click="handleSort('description')" class="sortable">
            <div class="th-content">
              Deskripsi
              <svg class="sort-icon" :class="{ active: sortConfig.column === 'description' }" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path :d="sortIcon('description')" />
              </svg>
            </div>
          </th>
          <th @click="handleSort('is_active')" class="sortable text-center" width="100">
      ...
      <tbody>
        ...
        <tr v-else-if="!items || items.length === 0">
          <td colspan="5" class="text-center py-4 text-muted">
            Tidak ada data komponen tindakan.
          </td>
        </tr>
